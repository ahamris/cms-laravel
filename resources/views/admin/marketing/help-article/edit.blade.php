<x-layouts.admin title="Edit Help Article">
<div class="space-y-6">
    {{-- Header --}}
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Help Article</h1>
            <p class="text-gray-600">Update {{ $helpArticle->title }}</p>
        </div>
    ...
    {{-- Form --}}
    <form action="{{ route('admin.marketing.help-article.update', $helpArticle) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <textarea id="excerpt"
                                      ...
                                      class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary focus:border-primary @error('excerpt') border-red-500 @enderror">{{ old('excerpt', $helpArticle->excerpt) }}</textarea>
                    ...
                    <div class="p-6">
                        <div>
                            ...
                            <textarea id="content"
                                      ...
                                      class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary focus:border-primary @error('content') border-red-500 @enderror">{{ old('content', $helpArticle->content) }}</textarea>
                                ...
                                @foreach($productFeatures as $feature)
                                    <div class="flex items-center">
                                        <input type="checkbox"
                                               ...
                                               value="{{ $feature->id }}"
                                               {{ in_array($feature->id, old('product_features', $helpArticle->productFeatures->pluck('id')->toArray())) ? 'checked' : '' }}
                    ...
                    <div class="p-6">
                        <div id="tags-container">
                            @if($helpArticle->tags && count($helpArticle->tags) > 0)
                                @foreach($helpArticle->tags as $index => $tag)
                                    <div class="tag-item flex items-center space-x-2 mb-2">
                                        <input type="text"
                                               name="tags[]"
                                               value="{{ old('tags.' . $index, $tag) }}"
                    ...
                    <div class="p-6 space-y-3">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                ...
                                @endswitch">
                                {{ ucfirst($helpArticle->difficulty_level) }}
                            </span>
                        </div>
                        <div class="flex items-center justify-between">
                        ...
                        @if($helpArticle->is_featured)
                            <div class="flex items-center justify-between">
                                ...
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                                    <i class="fa-solid fa-star mr-1"></i>Yes
                                </span>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <select id="difficulty_level"
                                ...
                                <option value="beginner" {{ old('difficulty_level', $helpArticle->difficulty_level) == 'beginner' ? 'selected' : '' }}>
                                    Beginner
                                </option>
                                <option value="intermediate" {{ old('difficulty_level', $helpArticle->difficulty_level) == 'intermediate' ? 'selected' : '' }}>
                                    Intermediate
                                </option>
                                <option value="advanced" {{ old('difficulty_level', $helpArticle->difficulty_level) == 'advanced' ? 'selected' : '' }}>
                                    Advanced
                                </option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number"
                                   ...
                                   value="{{ old('estimated_read_time', $helpArticle->estimated_read_time) }}"
                                   min="1"
                                   placeholder="e.g., 5"
                        ...
                        <div>
                            ...
                            <input type="number"
                                   id="sort_order"
                                   name="sort_order"
                                   value="{{ old('sort_order', $helpArticle->sort_order) }}"
                                   min="0"
                        ...
                        <div class="pt-4 border-t border-gray-200">
                            <div class="text-sm text-gray-600">
                                <p><strong>Created:</strong> {{ $helpArticle->created_at->format('M j, Y \a\t g:i A') }}</p>
                                @if($helpArticle->updated_at != $helpArticle->created_at)
                                    <p><strong>Updated:</strong> {{ $helpArticle->updated_at->format('M j, Y \a\t g:i A') }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   name="is_active"
                                   value="1"
                                   {{ old('is_active', $helpArticle->is_active) ? 'checked' : '' }}
                                   class="h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded">
