...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600">Update testimonial for {{ $testimonial->customer_name }}</p>
        </div>
    ...
    {{-- Form --}}
    <form action="{{ route('admin.marketing.testimonial.update', $testimonial) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <textarea id="quote"
                                      ...
                                      class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary focus:border-primary @error('quote') border-red-500 @enderror">{{ old('quote', $testimonial->quote) }}</textarea>
                        ...
                        <div>
                            ...
                            <div id="tags-container">
                                @if($testimonial->tags && count($testimonial->tags) > 0)
                                    @foreach($testimonial->tags as $index => $tag)
            ...
            <div class="lg:col-span-1 space-y-6">
                {{-- Current Images --}}
                @if($testimonial->photo || $testimonial->company_logo)
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                        <div class="px-6 py-4 border-b border-gray-200">
                        ...
                        <div class="p-6 space-y-4">
                            @if($testimonial->photo)
                                <div>
                                    ...
                                    <img src="{{ asset('storage/' . $testimonial->photo) }}" alt="{{ $testimonial->customer_name }}" class="w-16 h-16 rounded-full object-cover">
                                </div>
                            @endif
                            @if($testimonial->company_logo)
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Company Logo</label>
                                    <img src="{{ asset('storage/' . $testimonial->company_logo) }}" alt="{{ $testimonial->company }}" class="w-16 h-16 rounded object-contain bg-gray-50">
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="pt-4 border-t border-gray-200">
                            <div class="text-sm text-gray-600">
                                <p><strong>Created:</strong> {{ $testimonial->created_at->format('M j, Y \a\t g:i A') }}</p>
                                @if($testimonial->updated_at != $testimonial->created_at)
                                    <p><strong>Updated:</strong> {{ $testimonial->updated_at->format('M j, Y \a\t g:i A') }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
