<x-layouts.admin title="Edit Content Type">
<div class="space-y-6">
    {{-- Header --}}
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Content Type</h1>
            <p class="text-gray-600">Update content classification for {{ $contentType->name }}</p>
        </div>
    ...
    {{-- Form --}}
    <form action="{{ route('admin.marketing.content-type.update', $contentType) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        {{-- Slug --}}
                        <x-ui.input id="slug" name="slug" label="Slug" :value="old('slug', $contentType->slug)"
                        ...
                        <div>
                            ...
                            <textarea id="description"
                                      ...
                                      class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary focus:border-primary @error('description') border-red-500 @enderror">{{ old('description', $contentType->description) }}</textarea>
                    ...
                    <div class="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <div class="flex items-center space-x-2">
                                    <input type="color"
                                           id="color"
                                           name="color"
                                           value="{{ old('color', $contentType->color ?? '#6366f1') }}"
                        ...
                        <div class="border border-gray-200 rounded-lg p-4">
                            ...
                            <div id="preview" class="flex items-center">
                                <div id="preview-icon" class="h-10 w-10 rounded-lg flex items-center justify-center mr-3" style="background-color: {{ $contentType->color ?? '#6366f1' }}20;">
                                    <i id="preview-icon-element" class="fa-solid {{ $contentType->icon ?? 'fa-tag' }}" style="color: {{ $contentType->color ?? '#6366f1' }};"></i>
                                </div>
                                <div>
                                    <div id="preview-name" class="text-sm font-medium text-gray-900">{{ $contentType->name }}</div>
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center">
                                <input type="checkbox"
                                       ...
                                       {{ in_array('App\Models\Blog', old('applicable_models', $contentType->applicable_models ?? [])) ? 'checked' : '' }}
                            ...
                            <div class="flex items-center">
                                <input type="checkbox"
                                       ...
                                       {{ in_array('App\Models\Page', old('applicable_models', $contentType->applicable_models ?? [])) ? 'checked' : '' }}
                                       class="h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded">
                            ...
                            <div class="flex items-center">
                                <input type="checkbox"
                                       ...
                                       {{ in_array('App\Models\Service', old('applicable_models', $contentType->applicable_models ?? [])) ? 'checked' : '' }}
                                       class="h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded">
                            ...
                            <div class="flex items-center">
                                <input type="checkbox"
                                       ...
                                       {{ in_array('App\Models\CaseStudy', old('applicable_models', $contentType->applicable_models ?? [])) ? 'checked' : '' }}
                                       class="h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded">
                            ...
                            <div class="flex items-center">
                                <input type="checkbox"
                                       ...
                                       {{ in_array('App\Models\HelpArticle', old('applicable_models', $contentType->applicable_models ?? [])) ? 'checked' : '' }}
                                       class="h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded">
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    <div class="px-6 py-4 border-b border-gray-200">
                        <h3 class="text-lg font-semibold text-gray-900">Current Design</h3>
                    </div>
                    <div class="p-6 text-center">
                        <div class="flex items-center justify-center mb-4">
                            @if($contentType->icon)
                                <div class="h-16 w-16 rounded-lg flex items-center justify-center" style="background-color: {{ $contentType->color ?? '#6366f1' }}20;">
                                    <i class="fa-solid {{ $contentType->icon }} text-2xl" style="color: {{ $contentType->color ?? '#6366f1' }};"></i>
                                </div>
                            @else
                                <div class="h-16 w-16 rounded-lg bg-gray-100 flex items-center justify-center">
                                    <i class="fa-solid fa-tag text-gray-400 text-2xl"></i>
                                </div>
                            @endif
                        </div>
                        <p class="text-sm text-gray-600">Current appearance</p>
                    </div>
                </div>
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number"
                                   id="sort_order"
                                   name="sort_order"
                                   value="{{ old('sort_order', $contentType->sort_order) }}"
                                   min="0"
                        ...
                        <div class="pt-4 border-t border-gray-200">
                            <div class="text-sm text-gray-600">
                                <p><strong>Created:</strong> {{ $contentType->created_at->format('M j, Y \a\t g:i A') }}</p>
                                @if($contentType->updated_at != $contentType->created_at)
                                    <p><strong>Updated:</strong> {{ $contentType->updated_at->format('M j, Y \a\t g:i A') }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePreview() {
        // Update name
        previewName.textContent = nameInput.value || '{{ $contentType->name }}';
        
        // Update icon
        const iconClass = iconInput.value || '{{ $contentType->icon ?? 'fa-tag' }}';
        previewIconElement.className = `fa-solid ${iconClass}`;
        
        // Update color
        const color = colorInput.value || '{{ $contentType->color ?? '#6366f1' }}';
