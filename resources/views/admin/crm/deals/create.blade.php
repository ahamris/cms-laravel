<x-layouts.admin title="New Deal">
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="font-bold text-zinc-900 dark:text-white mb-2">New Deal</h1>
            <p class="text-zinc-600 dark:text-zinc-400">Create a new deal in the pipeline</p>
        </div>
    ...
    <form action="{{ route('admin.crm.deals.store') }}" method="POST" class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-6">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" required class="w-full rounded-md border-zinc-300 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white shadow-sm">
            ...
            <div>
                ...
                <select name="stage" id="stage" required class="w-full rounded-md border-zinc-300 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white shadow-sm">
                    @foreach(['lead' => 'Lead', 'qualified' => 'Qualified', 'proposal' => 'Proposal', 'negotiation' => 'Negotiation'] as $val => $label)
                        <option value="{{ $val }}" {{ old('stage', 'lead') === $val ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" name="value" id="value" value="{{ old('value') }}" min="0" class="w-full rounded-md border-zinc-300 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white shadow-sm">
            ...
            <div>
                ...
                <input type="number" name="probability" id="probability" value="{{ old('probability', 50) }}" min="0" max="100" class="w-full rounded-md border-zinc-300 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white shadow-sm">
            ...
            <div>
                ...
                <input type="date" name="expected_close_date" id="expected_close_date" value="{{ old('expected_close_date') }}" class="w-full rounded-md border-zinc-300 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white shadow-sm">
            </div>

            <div class="md:col-span-2">
                <label for="description" class="block font-medium text-zinc-700 dark:text-zinc-300 mb-1">Description</label>
                <textarea name="description" id="description" rows="3" class="w-full rounded-md border-zinc-300 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white shadow-sm">{{ old('description') }}</textarea>
            </div>
        </div>

        <div class="mt-6 flex items-center gap-3">
            <button type="submit" class="px-6 py-2 rounded-md bg-[var(--color-accent)] text-white font-semibold hover:opacity-90">Create Deal</button>
            <a href="{{ route('admin.crm.deals.index') }}" class="px-4 py-2 rounded-md border border-zinc-300 dark:border-zinc-600 text-zinc-700 dark:text-zinc-300 font-medium">Cancel</a>
        </div>
    </form>
</div>
</x-layouts.admin>
