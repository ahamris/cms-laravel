...
<x-layouts.admin title="{{ __('Create page') }}">
    <div class="space-y-6">
        ...
        <form id="admin-page-edit-form" action="{{ route('admin.content.page.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 rounded-lg border border-zinc-200 bg-zinc-100/90 p-4 shadow-sm dark:border-zinc-700 dark:bg-zinc-900/50"
            x-data="{
                templates: {{ Js::from($templates ?? []) }},
                currentTemplate: {{ Js::from($currentTemplate ?? 'default') }},
                dirty: false,
                isActiveLive: {{ Js::from((bool) old('is_active', true)) }},
                slugUnlocked: false,
                layoutRowExpanded: {},
                sidebarSeoOpen: true,
