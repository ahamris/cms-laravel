    ...
    <div class="space-y-6">
        <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="flex flex-col gap-1">
                <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">{{ __('Edit page') }}</h1>
                <p class="text-zinc-600 dark:text-zinc-400">{{ $page->title }}</p>
            </div>
            <div class="flex flex-wrap gap-2">
                <x-button variant="secondary" icon="arrow-left" icon-position="left" href="{{ route('admin.page.index') }}">
                    {{ __('Back to list') }}
                </x-button>
                <x-button variant="secondary" icon="eye" icon-position="left" href="{{ route('admin.page.show', $page) }}">
                    {{ __('View') }}
                </x-button>
            </div>
        </div>
