<x-layouts.admin title="Edit Subject">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-bold text-zinc-900 dark:text-white mb-2">Edit Subject</h1>
            <p class="text-zinc-600 dark:text-zinc-400">Update this contact form subject option.</p>
        </div>
    ...
    <form action="{{ route('admin.administrator.contact-subjects.update', $contactSubject) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="max-w-2xl">
            <div class="rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-6">
                <div class="mb-6">
                    ...
                    <p class="mt-1 text-sm/6 text-gray-600 dark:text-gray-400">Label shown in the contact form dropdown.</p>
                </div>
                <div class="space-y-6">
                    <div>
                        <x-ui.input
                            id="title"
                            name="title"
                            :value="old('title', $contactSubject->title)"
                            label="Title"
                    ...
                    <div>
                        <x-ui.input
                            ...
                            name="sort_order"
                            type="number"
                            :value="old('sort_order', $contactSubject->sort_order)"
                            label="Sort order"
                            min="0"
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <x-ui.toggle name="is_active" :checked="old('is_active', $contactSubject->is_active)" />
                    </div>
                </div>
            </div>
        </div>
