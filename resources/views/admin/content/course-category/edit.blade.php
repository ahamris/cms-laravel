<x-layouts.admin title="Edit Course Category">
    <form action="{{ route('admin.content.course-category.update', $courseCategory) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                ...
                <div class="rounded-md border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 p-6 shadow-sm">
                    ...
                    <div class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <x-ui.input name="name" id="name" label="Name" :value="old('name', $courseCategory->name)" required 
                                :error="$errors->has('name')" :errorMessage="$errors->first('name')" />
                            
                            <x-ui.input name="slug" id="slug" label="Slug" :value="old('slug', $courseCategory->slug)"
                        ...
                        <div class="space-y-2">
                            ...
                            <x-ui.textarea name="description" id="description" :value="old('description', $courseCategory->description)" :rows="4" 
            ...
            <div class="lg:col-span-1 space-y-8">
                ...
                <div class="rounded-md border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 p-6 shadow-sm">
                    ...
                    <x-image-upload id="image" name="image" label="Category Image" 
                        :current-image="$courseCategory->image_url"
                ...
                <div class="rounded-md border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 p-6 shadow-sm">
                    ...
                    <div class="space-y-4">
                        <x-ui.input name="sort_order" id="sort_order" label="Sort order" type="number"
                            :value="old('sort_order', $courseCategory->sort_order)" min="0" :error="$errors->has('sort_order')"
                            :errorMessage="$errors->first('sort_order')" />
                            
                        <div class="pt-2">
                            <x-ui.toggle name="is_active" label="Visible in academy" :checked="old('is_active', $courseCategory->is_active)" />
                        </div>
                    </div>
                </div>
            </div>
