<x-layouts.admin title="Create Feature Block">
<div class="space-y-6">
    {{-- Header --}}
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Create Feature Block</h1>
            <p class="text-gray-600">Create a new feature block with multiple items</p>
        </div>
    ...
    {{-- Form --}}
    <form action="{{ route('admin.content.block-feature.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" id="featureBlockForm">
        @csrf
...
function addItem() {
    ...
    const itemHtml = `
        <div class="border border-gray-200 rounded-lg p-4 relative" id="item-${itemIndex}">
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <div class="flex justify-center px-4 py-4 border-2 border-gray-300 border-dashed rounded-lg hover:border-primary transition-colors duration-200 cursor-pointer" onclick="document.getElementById('item_image_${itemIndex}').click()">
                        <div class="space-y-1 text-center">
                            <div id="imagePreview-${itemIndex}" class="hidden">
                                <img class="mx-auto h-24 w-auto rounded-lg" src="" alt="Preview">
                            </div>
                            <div id="imagePlaceholder-${itemIndex}">
                    ...
                    <input id="item_image_${itemIndex}" 
                           name="item_images[${itemIndex}]" 
                           type="file" 
                           class="hidden" 
                           accept="image/*"
                           onchange="previewImage(${itemIndex}, this)">
                </div>
                ...
                <div class="col-span-2 space-y-3">
                    ...
                    <div>
                        ...
                        <textarea name="items[${itemIndex}][content]"
                                  rows="4"
                                  placeholder="Enter item content"
                                  class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary focus:border-primary"></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-3 mt-3">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="relative flex items-center p-3 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-primary transition-colors duration-200 group">
                            <input type="radio" 
                                   name="items[${itemIndex}][image_position]" 
                                   value="left" 
                                   checked
                                   class="sr-only peer">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text"
                               name="items[${itemIndex}][link_text]"
                    ...
                    <div>
                        ...
                        <div x-data="urlSelector${itemIndex}()" class="space-y-2">
                            <div class="grid grid-cols-3 gap-2 mb-2">
                                <label class="flex items-center space-x-1 cursor-pointer">
                                    <input type="radio" name="items[${itemIndex}][link_type]" value="predefined" 
                                           x-model="linkType"
                                           class="w-3 h-3 text-primary border-gray-300 focus:ring-primary">
                                    <span class="text-xs text-gray-700">Route</span>
                                </label>
                                <label class="flex items-center space-x-1 cursor-pointer">
                                    <input type="radio" name="items[${itemIndex}][link_type]" value="system" 
                                           x-model="linkType"
                                           class="w-3 h-3 text-primary border-gray-300 focus:ring-primary">
                                    <span class="text-xs text-gray-700">Content</span>
                                </label>
                                <label class="flex items-center space-x-1 cursor-pointer">
                                    <input type="radio" name="items[${itemIndex}][link_type]" value="custom" 
                                           x-model="linkType"
                                           class="w-3 h-3 text-primary border-gray-300 focus:ring-primary">
                            ...
                            <div x-show="linkType === 'predefined'" x-transition class="space-y-1">
                                <select x-model="selectedRoute" @change="updateUrl()" 
                                        id="predefined_route_${itemIndex}"
                            ...
                            <input type="hidden" name="items[${itemIndex}][link_url]" x-model="finalUrl">
                        </div>
                    </div>
                </div>
            </div>
