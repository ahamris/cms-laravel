<x-layouts.admin title="Edit TailwindPlus Component">
<div class="space-y-6">
    {{-- Header --}}
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col gap-1">
                ...
                <p>Update Tailwind UI component</p>
            </div>
        </div>
        <div class="flex items-center space-x-3">
            <a href="{{ route('admin.content.tailwind-plus.show', $tailwindPlus) }}"
               class="px-5 py-2 text-sm bg-white border border-gray-200 rounded-md text-gray-700 hover:bg-gray-50 transition-colors duration-200 flex items-center space-x-2">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        <form action="{{ route('admin.content.tailwind-plus.update', $tailwindPlus) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="category"
                                ...
                                <option value="MARKETING" {{ old('category', $tailwindPlus->category) === 'MARKETING' ? 'selected' : '' }}>MARKETING</option>
                                <option value="APPLICATION UI" {{ old('category', $tailwindPlus->category) === 'APPLICATION UI' ? 'selected' : '' }}>APPLICATION UI</option>
                                <option value="ECOMMERCE" {{ old('category', $tailwindPlus->category) === 'ECOMMERCE' ? 'selected' : '' }}>ECOMMERCE</option>
                            </select>
                            @error('category')
                        ...
                        {{-- Component Group --}}
                        <div x-data="{ showNewGroupInput: false, selectedGroup: '{{ old('component_group', $tailwindPlus->component_group) ?? '' }}', newGroup: '' }">
                            ...
                            <div class="flex gap-2">
                                <select x-model="selectedGroup"
                                    ...
                                    @foreach($componentGroups ?? [] as $group)
                                        <option value="{{ $group }}" {{ old('component_group', $tailwindPlus->component_group) === $group ? 'selected' : '' }}>{{ $group }}</option>
                                    @endforeach
                                </select>
                                <button type="button"
                                        @click="showNewGroupInput = !showNewGroupInput; if (!showNewGroupInput) { selectedGroup = '{{ old('component_group', $tailwindPlus->component_group) ?? '' }}'; newGroup = ''; }"
                        ...
                        <div>
                            ...
                            <input type="text"
                                   id="component_name"
                                   name="component_name"
                                   value="{{ old('component_name', $tailwindPlus->component_name) }}"
                        ...
                        <div>
                            ...
                            <input type="number"
                                   id="version"
                                   name="version"
                                   value="{{ old('version', $tailwindPlus->version) }}"
                                   min="1"
                        ...
                        <div>
                            ...
                            <x-ui.toggle name="is_active" label="Active" :checked="old('is_active', $tailwindPlus->is_active)" />
                            @error('is_active')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="code"
                                      ...
                                      required>{{ old('code', $tailwindPlus->code) }}</textarea>
                            @error('code')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea id="preview"
                                      ...
                                      placeholder="Optional preview description or notes">{{ old('preview', $tailwindPlus->preview) }}</textarea>
                            @error('preview')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
