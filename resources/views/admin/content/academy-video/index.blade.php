<x-layouts.admin title="Document Videos">
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.content.academy-video.create') }}">
                <x-button variant="primary" icon="plus" icon-position="left">Add Video</x-button>
            </a>
        </div>

        <livewire:admin.table
            resource="academy-video"
            :columns="[
                'id',
                ['key' => 'category.name', 'label' => 'Category'],
                'title',
                'slug',
                ['key' => 'duration_seconds', 'label' => 'Duration', 'type' => 'custom', 'view' => 'admin.content.academy-video.partials.duration-cell'],
                ['key' => 'is_active', 'type' => 'toggle'],
                ['key' => 'created_at', 'format' => 'date'],
            ]"
            route-prefix="admin.content.academy-video"
            search-placeholder="Search videos..."
            :paginate="15"
            custom-actions-view="admin.content.academy-video.partials.table-actions"
