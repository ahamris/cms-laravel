<x-layouts.admin title="Edit Academy Video">
    {{-- Page Header --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-bold text-zinc-900 dark:text-white mb-2">Edit Academy Video</h1>
            <p class="text-zinc-600 dark:text-zinc-400">{{ $academyVideo->title }}</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.content.academy-video.show', $academyVideo) }}">
    ...
    <form action="{{ route('admin.content.academy-video.update', $academyVideo) }}" method="POST"
        enctype="multipart/form-data" id="academy-video-form">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div
                    ...
                    <div class="space-y-6" x-data="academyChapterForm()"
                        data-chapters-url="{{ route('admin.content.academy-chapters.by-category') }}">
                        <x-ui.select name="academy_category_id" label="Category" :options="['' => 'Select category'] + $categories->mapWithKeys(fn($c) => [$c->id => $c->name])->all()" :value="(string) old('academy_category_id', $academyVideo->academy_category_id)" required
                            :error="$errors->has('academy_category_id')"
                            :errorMessage="$errors->first('academy_category_id')" />

                        <x-ui.select name="academy_chapter_id" id="academy_chapter_id" label="Chapter" :options="$chapterOptions" :value="(string) old('academy_chapter_id', $academyVideo->academy_chapter_id ?? '')" :error="$errors->has('academy_chapter_id')" :errorMessage="$errors->first('academy_chapter_id')" />

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <x-ui.input name="title" id="title" label="Title" :value="old('title', $academyVideo->title)" required :error="$errors->has('title')"
                                :errorMessage="$errors->first('title')" />
                            <x-ui.input name="slug" id="slug" label="Slug" :value="old('slug', $academyVideo->slug)"
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="prose-container">
                                <x-editor id="description" name="description" :value="old('description', $academyVideo->description)" placeholder="Video description..." />
                            </div>
                ...
                <div
                    ...
                    <div class="space-y-6">
                        ...
                        <x-ui.input name="video_url" id="video_url" label="External video URL" type="url"
                            :value="old('video_url', $academyVideo->video_url)" placeholder="https://..."
            ...
            <div class="lg:col-span-1 space-y-8">
                ...
                <div
                    ...
                    <div class="space-y-4">
                        ...
                        <a href="{{ route('admin.content.academy-video.index') }}" class="block">
                ...
                <div
                    ...
                    <x-image-upload id="thumbnail" name="thumbnail" label="Cover image" :required="false"
                        ...
                        :current-image="$academyVideo->thumbnail_path ? $academyVideo->thumbnail_url : null"
                        :current-image-alt="$academyVideo->title" />
                </div>
                ...
                <div
                    ...
                    <div class="space-y-4">
                        <x-ui.input name="duration_seconds" id="duration_seconds" label="Duration (seconds)"
                            type="number" :value="old('duration_seconds', $academyVideo->duration_seconds)" min="0"
                        ...
                        <x-ui.input name="sort_order" id="sort_order" label="Sort order" type="number"
                            :value="old('sort_order', $academyVideo->sort_order)" min="0"
                            :error="$errors->has('sort_order')" :errorMessage="$errors->first('sort_order')" />

                        <div class="pt-2">
                            <x-ui.toggle name="is_active" label="Visible on academy" :checked="old('is_active', $academyVideo->is_active)" />
                        </div>
                    </div>
                </div>
            </div>
    ...
    @include('admin.content.academy-video.partials.chapter-form-script')
</x-layouts.admin>