<x-layouts.admin title="Add Academy Video">
    {{-- Page Header --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-bold text-zinc-900 dark:text-white mb-2">Add Academy Video</h1>
    ...
    <form action="{{ route('admin.content.academy-video.store') }}" method="POST" enctype="multipart/form-data"
        id="academy-video-form">
        @csrf
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div
                    ...
                    <div class="space-y-6" x-data="academyChapterForm()"
                        ...
                        <x-ui.select name="academy_category_id" label="Category" :options="['' => 'Select category'] + $categories->mapWithKeys(fn($c) => [$c->id => $c->name])->all()" :value="(string) old('academy_category_id')" required :error="$errors->has('academy_category_id')"
                            :errorMessage="$errors->first('academy_category_id')" />

                        <x-ui.select name="academy_chapter_id" id="academy_chapter_id" label="Chapter" :options="$chapterOptions" :value="(string) old('academy_chapter_id', '')" :error="$errors->has('academy_chapter_id')" :errorMessage="$errors->first('academy_chapter_id')" />

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <x-ui.input name="title" id="title" label="Title" :value="old('title')" required
                                :error="$errors->has('title')" :errorMessage="$errors->first('title')" />
                            <x-ui.input name="slug" id="slug" label="Slug" :value="old('slug')"
                                placeholder="url-friendly-title" required slug-from="title"
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="prose-container">
                                <x-editor id="description" name="description" :value="old('description')"
                ...
                <div
                    ...
                    <div class="mb-6">
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Upload a file or provide an external
                            URL (e.g. YouTube, Vimeo). Leave both empty to create <strong>plain documentation</strong> (no video).</p>
                    </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <x-ui.input name="video_url" id="video_url" label="External video URL" type="url"
                            :value="old('video_url')" placeholder="https://..." :error="$errors->has('video_url')"
                            :errorMessage="$errors->first('video_url')" />
                    </div>
                </div>
            </div>
                ...
                <div
                    ...
                    <div class="space-y-4">
                        <x-button variant="primary" type="submit" class="w-full justify-center" icon="save"
                            icon-position="left">Create Video</x-button>
                ...
                <div
                    ...
                    <div class="space-y-4">
                        <x-ui.input name="duration_seconds" id="duration_seconds" label="Duration (seconds)"
                            type="number" :value="old('duration_seconds')" min="0" placeholder="e.g. 600"
                        ...
                        <div class="pt-2">
                            <x-ui.toggle name="is_active" label="Visible on academy" :checked="old('is_active', true)" />
                        </div>
                    </div>
                </div>
            </div>
