@if(in_array('view', $actions))
    <a href="{{ route('admin.content.presenter.show', $item) }}" title="View">
        <x-button variant="sky" size="sm" icon="eye" title="View"></x-button>
    </a>
@endif

@if(in_array('edit', $actions))
    <a href="{{ route('admin.content.presenter.edit', $item) }}" title="Edit">
...
@if(in_array('delete', $actions))
    @php
        $modelName = class_basename($this->modelClass ?? \App\Models\Presenter::class);
    @endphp
    <x-button
        variant="error"
        size="sm"
    ...
    <x-ui.modal modal-id="delete-modal-{{ $item->id }}" alpine-show="showDeleteModal{{ $item->id }}" size="sm">
        ...
        <div class="space-y-4">
            <p class="text-zinc-600 dark:text-zinc-400">
                Are you sure you want to delete this presenter? This action cannot be undone.
            </p>
