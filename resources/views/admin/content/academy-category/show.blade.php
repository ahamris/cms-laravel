<x-layouts.admin title="Category: {{ $academyCategory->name }}">


    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $academyCategory->name }}</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Academy Category Details</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.content.academy-category.edit', $academyCategory) }}">
                <x-button variant="secondary" icon="pencil">Edit Category</x-button>
            </a>
            <form action="{{ route('admin.content.academy-category.destroy', $academyCategory) }}" method="POST"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900 dark:text-gray-200 whitespace-pre-wrap">
                            {{ $academyCategory->description ?: 'No description available.' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-md border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 p-6 shadow-sm">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <a href="{{ route('admin.content.academy-video.index') }}?category={{ $academyCategory->id }}">
                        <x-button variant="secondary" size="sm" icon="external-link">Manage Videos</x-button>
                    </a>
                </div>

                @if($academyCategory->videos->count() > 0)
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-zinc-200 dark:divide-white/10">
                            ...
                            <tbody class="divide-y divide-zinc-200 dark:divide-white/10 bg-white dark:bg-transparent">
                                @foreach($academyCategory->videos->take(5) as $video)
                                    <tr>
                                        ...
                                        <td class="px-3 py-3 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('admin.content.academy-video.show', $video) }}"
                    ...
                    @if($academyCategory->videos->count() > 5)
                        <div class="mt-4 text-center">
                            <p class="text-sm text-gray-500">Showing 5 of {{ $academyCategory->videos->count() }} videos.</p>
                        </div>
                    @endif
                @else
        ...
        <div class="lg:col-span-1 space-y-8">
            ...
            <div class="rounded-md border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 p-6 shadow-sm">
                ...
                @if($academyCategory->image_path)
                    <img src="{{ asset('storage/' . $academyCategory->image_path) }}" alt="{{ $academyCategory->name }}"
            ...
            <div class="rounded-md border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 p-6 shadow-sm">
                ...
                <dl class="space-y-3 text-sm">
                    <div>
                        ...
                        <dd class="font-mono text-zinc-900 dark:text-white break-all">{{ $academyCategory->slug }}</dd>
                    </div>
                    <div>
                        <dt class="text-zinc-500 dark:text-zinc-400">Status</dt>
                        <dd class="mt-1"><x-ui.badge :variant="$academyCategory->is_active ? 'success' : 'secondary'">{{ $academyCategory->is_active ? 'Active' : 'Inactive' }}</x-ui.badge></dd>
                    </div>
                    <div>
                        <dt class="text-zinc-500 dark:text-zinc-400">Sort Order</dt>
                        <dd class="text-zinc-900 dark:text-white">{{ $academyCategory->sort_order }}</dd>
                    </div>
                    <div>
                        <dt class="text-zinc-500 dark:text-zinc-400">Total Videos</dt>
                        <dd class="text-zinc-900 dark:text-white">{{ $academyCategory->videos->count() }}</dd>
                    </div>
                </dl>
            </div>
        </div>
