<x-layouts.admin title="Create Page Block Preset">
<div class="space-y-6">
    {{-- Header --}}
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Create Page Block Preset</h1>
            <p class="text-gray-600">Create a new block preset for showcase pages</p>
        </div>
    ...
    {{-- Form --}}
    <form action="{{ route('admin.content.page-block-preset.store') }}" method="POST" class="space-y-6">
        @csrf
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <select id="type"
                                ...
                                <option value="body" {{ old('type') === 'body' ? 'selected' : '' }}>Body</option>
                            </select>
                            @error('type')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea id="blocks"
                                      ...
                                      required
                                      class="w-full border border-gray-300 rounded-lg px-3 py-2 font-mono text-xs focus:ring-2 focus:ring-primary focus:border-primary @error('blocks') border-red-500 @enderror">{{ old('blocks', '[]') }}</textarea>
                            @error('blocks')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
