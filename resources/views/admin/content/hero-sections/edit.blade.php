...
@endphp
<x-layouts.admin title="Edit Hero Section">
<div class="space-y-6">
    {{-- Header --}}
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Hero Section</h1>
            <p class="text-gray-600">Update hero section content and settings</p>
        </div>
    ...
    {{-- Form --}}
    <form action="{{ route('admin.content.hero-section.update', $heroSection) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <textarea id="subtitle"
                                      name="subtitle"
                                      rows="3"
                                      class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary focus:border-primary @error('subtitle') border-red-500 @enderror">{{ old('subtitle', $heroSection->subtitle) }}</textarea>
                            @error('subtitle')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text"
                                   id="slogan"
                                   name="slogan"
                                   value="{{ old('slogan', $heroSection->slogan) }}"
                        ...
                        <div>
                            ...
                            <div id="no-items-state" class="text-center py-8 border-2 border-dashed border-gray-300 rounded-lg {{ ($heroSection->list_items && count($heroSection->list_items) > 0) ? 'hidden' : '' }}">
                            ...
                            <div id="list-items-form" class="{{ ($heroSection->list_items && count($heroSection->list_items) > 0) ? '' : 'hidden' }}">
                                <div id="list-items-container" class="space-y-2" style="list-style: none;">
                                    @if($heroSection->list_items && count($heroSection->list_items) > 0)
                                        @foreach($heroSection->list_items as $index => $item)
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <x-icon-picker
                                    ...
                                    :value="old('top_header_icon', $heroSection->top_header_icon)"
                            ...
                            <div>
                                ...
                                <input type="text"
                                       ...
                                       value="{{ old('top_header_text', $heroSection->top_header_text) }}"
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        <x-icon-picker
                                            id="card1_icon"
                                            name="card1_icon"
                                            :value="old('card1_icon', $heroSection->card1_icon)"
                                            label="Icon"
                                            help-text=""
                                            :required="false"
                                        />
                                    ...
                                    <div>
                                        ...
                                        <select id="card1_bgcolor"
                                                name="card1_bgcolor"
                                                class="w-full border border-gray-300 rounded px-2 py-1 text-sm focus:ring-1 focus:ring-primary focus:border-primary">
                                            <option value="bg-primary" {{ old('card1_bgcolor', $heroSection->card1_bgcolor) == 'bg-primary' ? 'selected' : '' }}>Primary</option>
                                            <option value="bg-secondary" {{ old('card1_bgcolor', $heroSection->card1_bgcolor) == 'bg-secondary' ? 'selected' : '' }}>Secondary</option>
                                        </select>
                                    </div>
                                    <div>
                                        ...
                                        <input type="text"
                                               id="card1_title"
                                               name="card1_title"
                                               value="{{ old('card1_title', $heroSection->card1_title) }}"
                                    ...
                                    <div>
                                        ...
                                        <input type="text"
                                               ...
                                               value="{{ old('card1_description', $heroSection->card1_description) }}"
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        <x-icon-picker
                                            id="card2_icon"
                                            name="card2_icon"
                                            :value="old('card2_icon', $heroSection->card2_icon)"
                                            label="Icon"
                                            help-text=""
                                            :required="false"
                                        />
                                    ...
                                    <div>
                                        ...
                                        <select id="card2_bgcolor"
                                                name="card2_bgcolor"
                                                class="w-full border border-gray-300 rounded px-2 py-1 text-sm focus:ring-1 focus:ring-primary focus:border-primary">
                                            <option value="bg-primary" {{ old('card2_bgcolor', $heroSection->card2_bgcolor) == 'bg-primary' ? 'selected' : '' }}>Primary</option>
                                            <option value="bg-secondary" {{ old('card2_bgcolor', $heroSection->card2_bgcolor) == 'bg-secondary' ? 'selected' : '' }}>Secondary</option>
                                        </select>
                                    </div>
                                    <div>
                                        ...
                                        <input type="text"
                                               id="card2_title"
                                               name="card2_title"
                                               value="{{ old('card2_title', $heroSection->card2_title) }}"
                                               placeholder="Card title"
                                               class="w-full border border-gray-300 rounded px-2 py-1 text-sm focus:ring-1 focus:ring-primary focus:border-primary">
                        </div>
                        <div>
                            ...
                            <input type="text"
                                               ...
                                               value="{{ old('card2_description', $heroSection->card2_description) }}"
                                               placeholder="Short description"
                                               class="w-full border border-gray-300 rounded px-2 py-1 text-sm focus:ring-1 focus:ring-primary focus:border-primary">
                                    </div>
                                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    ...
                    <div class="p-6">
                        <x-image-upload
                            ...
                            :max-size="2048"
                            :current-image="$heroSection->image ? Storage::disk('public')->url($heroSection->image) : null"
                            :current-image-alt="'Hero section image'"
                        />
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div>
                            <x-ui.toggle 
                                name="is_active"
                                :checked="old('is_active', $heroSection->is_active)"
...
<script>
let listItemIndex = {{ count($heroSection->list_items ?? []) }};
...
function cleanupEmptyListItems() {
    ...
    // Check if no items remain
    const remainingItems = container.querySelectorAll('div.flex');
    console.log('Remaining items after cleanup:', remainingItems.length);
    
    if (remainingItems.length === 0) {
        console.log('No items remain, adding clear flag');
        // Add hidden input to indicate list should be cleared
        const hiddenInput = document.createElement('input');
        hiddenInput.type = 'hidden';
        hiddenInput.name = 'list_items_clear';
        ...
        hiddenInput.id = 'list_items_clear_flag';
        container.appendChild(hiddenInput);
        console.log('Clear flag added to form');
    }
}

function topHeaderUrlSelector() {
    const currentUrl = '{{ old('top_header_url', $heroSection->top_header_url ?? '') }}';
...
function primaryUrlSelector() {
    const currentUrl = '{{ old('primary_button_url', $heroSection->primary_button_url ?? '') }}';
...
function secondaryUrlSelector() {
    const currentUrl = '{{ old('secondary_button_url', $heroSection->secondary_button_url ?? '') }}';
    const availableRoutes = @json($availableRoutes ?? []);
    const systemContent = @json($systemContent ?? []);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            console.log('Form submitting, cleaning up list items');
