<x-layouts.admin title="Create Page">
    <div id="page-builder-app" v-cloak x-ignore>
            ...
            <form action="{{ route('admin.content.page.store') }}" method="POST" enctype="multipart/form-data"
                            ...
                            <div v-show="isStatic" class="space-y-6">
                                ...
                                <div class="bg-white rounded-md border border-gray-200">
                                    <div class="p-6 space-y-6">
                                        ...
                                        <div>
                                            <label class="block text-xs font-medium text-gray-700 mb-1">Long Body <span
                                                    class="text-red-500">*</span></label>
                                            {{-- TipTap Editor for Vue.js --}}
                                            <x-editor-vue id="long_body" name="long_body"
                                                placeholder="Write the full content here..." :value="old('long_body', '')" />
                                            <p v-if="errors.long_body" class="mt-1 text-sm text-red-600"
                                                v-text="errors.long_body[0]"></p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div v-if="modals.spacingEdit.show"
                                ...
                                <div
                                    ...
                                    <div class="p-6 space-y-6">
                                        ...
                                        <div class="mb-4">
                                            <div class="flex border-b border-gray-200">
                                                <button v-for="bp in spacingBreakpoints" :key="bp.key" type="button"
                                                    @click="activeSpacingBreakpoint = bp.key" :class="{
                ...
                <div class="flex justify-end space-x-3 pt-6">
                    ...
                    <button type="submit"
                        ...
                        <span class="text-sm">Create Page</span>
                    </button>
                </div>
            </form>
        </div>
        ...
        <x-slot:styles>
            ...
            {{-- TipTap (editor-vue) styles are loaded by the layout/component --}}
            <style>
                [v-cloak] {
                    display: none;
                }

                /* Ensure disabled images (hidden on frontend) are visible but dimmed in editor */
                .preview-content-wrapper .hidden {
            ...
            <script>
                ...
                window.PageBuilderData = {
                    form: {
                        ...
                        slug: @json(old('slug', '')),
                        is_active: @json(old('is_active', true)),
                        home_page: @json(old('home_page', false)),
                        page_type: @json(old('page_type', 'static')),

                        short_body: @json(old('short_body', '')),
                        long_body: @json(old('long_body', '')),

                        layout_type: @json(old('layout_type', '')),
                        design_type: @json(old('design_type', 'general')),
                        header_block: @json(old('header_block', '')),
                        footer_block: @json(old('footer_block', '')),
                        hide_header: @json(old('hide_header', false)),
                        hide_footer: @json(old('hide_footer', false)),

                        funnel_fase: @json(old('funnel_fase', '')),
                        marketing_persona_id: @json(old('marketing_persona_id', '')),
                        content_type_id: @json(old('content_type_id', '')),
                        primary_keyword: @json(old('primary_keyword', '')),
                        secondary_keywords: @json(old('secondary_keywords', [])),
                        ai_briefing: @json(old('ai_briefing', '')),

                        meta_title: @json(old('meta_title', '')),
                        meta_body: @json(old('meta_body', '')),
                        meta_keywords: @json(old('meta_keywords', '')),
                    },
                    blocks: @json(json_decode(old('widget_config', '[]'))),
                    resources: {
