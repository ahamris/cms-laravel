    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between mb-6">
            <div class="flex flex-col gap-1">
                ...
                <p class="text-gray-600">Add a new page</p>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-gray-50/50 rounded-md border border-gray-200 p-6">
                        ...
                        <p class="text-xs text-gray-500 mb-3">Optional featured image.</p>
                    ...
                    <div class="bg-gray-50/50 rounded-md border border-gray-200 p-6 space-y-6">
                        <div>
                            ...
                            <textarea id="short_body" name="short_body" rows="4" required
                                ...
                                placeholder="Brief summary">{{ old('short_body') }}</textarea>
                            @error('short_body')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            ...
                            <x-editor id="long_body" name="long_body" :value="old('long_body')" placeholder="Write the full content here..." />
                            @error('long_body')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select id="funnel_fase" name="funnel_fase"
                                    ...
                                    <option value="interesseer" @selected(old('funnel_fase') === 'interesseer')>Interesseer</option>
                                    <option value="overtuig" @selected(old('funnel_fase') === 'overtuig')>Overtuig</option>
                                    <option value="activeer" @selected(old('funnel_fase') === 'activeer')>Activeer</option>
                                    <option value="inspireer" @selected(old('funnel_fase') === 'inspireer')>Inspireer</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select id="marketing_persona_id" name="marketing_persona_id"
                                    ...
                                    @foreach($marketingPersonas as $persona)
                                        <option value="{{ $persona->id }}" @selected(old('marketing_persona_id') == $persona->id)>{{ $persona->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select id="content_type_id" name="content_type_id"
                                    ...
                                    @foreach($contentTypes as $type)
                                        <option value="{{ $type->id }}" @selected(old('content_type_id') == $type->id)>{{ $type->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <label for="primary_keyword" class="block text-xs font-medium text-gray-700 mb-1">Primary Keyword</label>
                                <input type="text" id="primary_keyword" name="primary_keyword" value="{{ old('primary_keyword') }}"
                                    class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md focus:outline-none">
                            </div>
                            <div class="md:col-span-2">
                                ...
                                <textarea id="ai_briefing" name="ai_briefing" rows="3"
                                    class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md focus:outline-none">{{ old('ai_briefing') }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-gray-50/50 rounded-md border border-gray-200 p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <textarea id="meta_body" name="meta_body" rows="3"
                                class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md focus:outline-none">{{ old('meta_body') }}</textarea>
                        </div>
                        <div>
                            ...
                            <textarea id="meta_keywords" name="meta_keywords" rows="3"
                                class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md focus:outline-none">{{ old('meta_keywords') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
