<x-layouts.admin title="Create Documentation Page">
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col gap-1">
                <h2>Create Documentation Page</h2>
                <p>Add a page to a section</p>
            </div>
        </div>
    ...
    <div class="bg-gray-50/50 rounded-md border border-gray-200">
        <form action="{{ route('admin.doc-pages.store') }}" method="POST" class="space-y-6">
            @csrf
                    ...
                    <div class="bg-gray-50/50 rounded-md border border-gray-200 p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="title" name="title" value="{{ old('title') }}" placeholder="Page title" class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md focus:outline-none @error('title') border-red-500 @enderror" required>
                                @error('title')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                            </div>
                            <x-ui.input id="slug" name="slug" label="Slug" :value="old('slug')" placeholder="page-slug" slug-from="title" hint="Auto-generated if left blank." :error="$errors->has('slug')" :errorMessage="$errors->first('slug')" />
                            <div>
                                <label for="content" class="block text-sm font-medium text-gray-700 mb-2">Content <span class="text-red-500">*</span></label>
                                <x-editor id="content" name="content" :value="old('content')" placeholder="Page content..." />
                    ...
                    <div class="bg-gray-50/50 rounded-md border border-gray-200 p-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" id="meta_title" name="meta_title" value="{{ old('meta_title') }}" class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md focus:outline-none @error('meta_title') border-red-500 @enderror">
                            ...
                            <div>
                                ...
                                <textarea id="meta_description" name="meta_description" rows="2" class="w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-md focus:outline-none @error('meta_description') border-red-500 @enderror">{{ old('meta_description') }}</textarea>
            ...
            <div class="px-6 py-4 bg-gray-50/80 border-t border-gray-200 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="px-5 py-2 text-sm text-white bg-primary rounded-md hover:bg-primary/80">Create Page</button>
            </div>
        </form>
    </div>
</x-layouts.admin>
