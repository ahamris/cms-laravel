@if(in_array('view', $actions))
    <a href="{{ route('admin.content.live-session.show', $item) }}" title="View">
        <x-button variant="sky" size="sm" icon="eye" title="View"></x-button>
    </a>
@endif

@if(in_array('edit', $actions))
    <a href="{{ route('admin.content.live-session.edit', $item) }}" title="Edit">
...
@if(in_array('delete', $actions))
    @php
        $modelName = class_basename($this->modelClass ?? \App\Models\LiveSession::class);
    @endphp
    <x-button
        variant="error"
        size="sm"
            ...
            <div class="bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4 space-y-2">
                ...
                @if($item->session_date)
                    <div class="text-sm">
                        ...
                        <span class="text-zinc-700 dark:text-zinc-300 ml-1">{{ $item->formatted_date }}</span>
                    </div>
                @endif
            </div>
        </div>
