<x-layouts.admin title="Edit Contact">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            ...
            <p class="text-zinc-600 dark:text-zinc-400">Update {{ $contact->organization_name }}</p>
        </div>
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="{{ route('admin.administrator.contacts.show', $contact) }}" class="inline-flex items-center gap-2 rounded-md bg-white dark:bg-white/10 px-4 py-2 text-sm font-semibold text-gray-900 dark:text-white shadow-xs ring-1 ring-gray-300 ring-inset dark:ring-white/10 hover:bg-gray-50 dark:hover:bg-white/20">
    ...
    <form action="{{ route('admin.administrator.contacts.update', $contact) }}" method="POST" id="contactForm">
        @csrf
        @method('PUT')
                ...
                <div class="rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-6">
                    ...
                    <div class="space-y-6">
                        <div
                            class="space-y-6"
                            x-data="{
                                fieldsLocked: @js($organizationFieldsLocked),
                                generateSlug(text) {
                                    if (!text) return '';
                            ...
                            x-init="if (fieldsLocked) { $nextTick(() => { lockFields(); }); }"
                ...
                <div class="rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <x-ui.input
                            ...
                            :value="old('chamber_of_commerce', $contact->chamber_of_commerce)"
                ...
                <div class="rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <x-ui.input
                            ...
                            label="Invoice Email"
                            placeholder="invoices@organization.com"
                        />
                        ...
                        <x-ui.input
                            ...
                            :value="old('payment_due_days', $contact->payment_due_days)"
                        ...
                        {{-- Currency --}}
                        <div>
                            <label for="currency" class="block text-sm/6 font-medium text-gray-900 dark:text-white">Currency <span class="text-red-500">*</span></label>
                            <div class="mt-2 grid grid-cols-1">
                                <select id="currency" 
                                    ...
                                    <option value="EUR" {{ old('currency', $contact->currency) == 'EUR' ? 'selected' : '' }}>EUR - Euro</option>
                                    <option value="USD" {{ old('currency', $contact->currency) == 'USD' ? 'selected' : '' }}>USD - US Dollar</option>
                                    <option value="GBP" {{ old('currency', $contact->currency) == 'GBP' ? 'selected' : '' }}>GBP - British Pound</option>
                                </select>
                        ...
                        {{-- Preferred Language --}}
                        <div>
                            <label for="preferred_language" class="block text-sm/6 font-medium text-gray-900 dark:text-white">Preferred Language <span class="text-red-500">*</span></label>
                            <div class="mt-2 grid grid-cols-1">
                                <select id="preferred_language" 
                                    ...
                                    <option value="nl" {{ old('preferred_language', $contact->preferred_language) == 'nl' ? 'selected' : '' }}>Dutch</option>
                                    <option value="en" {{ old('preferred_language', $contact->preferred_language) == 'en' ? 'selected' : '' }}>English</option>
                                    <option value="de" {{ old('preferred_language', $contact->preferred_language) == 'de' ? 'selected' : '' }}>German</option>
                                    <option value="fr" {{ old('preferred_language', $contact->preferred_language) == 'fr' ? 'selected' : '' }}>French</option>
                                </select>
                                <svg viewBox="0 0 16 16" fill="currentColor" class="pointer-events-none col-start-1 row-start-1 mr-2 size-5 self-center justify-self-end text-gray-500 sm:size-4 dark:text-gray-400">
                                    <path d="M4.22 6.22a.75.75 0 0 1 1.06 0L8 8.94l2.72-2.72a.75.75 0 1 1 1.06 1.06l-3.25 3.25a.75.75 0 0 1-1.06 0L4.22 7.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" fill-rule="evenodd" />
                                </svg>
                            </div>
                            @error('preferred_language')<p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                        </div>
                    </div>
                </div>

                {{-- Notes Section --}}
                <div class="rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-6 pb-24 lg:pb-6">
                    <div class="mb-6">
            ...
            <div class="space-y-8">
                ...
                <div class="rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-6">
                    <div class="mb-6">
                        ...
                        <p class="mt-1 text-sm/6 text-gray-600 dark:text-gray-400">Configure contact type and visibility.</p>
                    </div>

                    <div class="space-y-6">
                        {{-- Is Customer --}}
                        <div>
                            <label for="is_customer" class="block text-sm/6 font-medium text-gray-900 dark:text-white">Customer <span class="text-red-500">*</span></label>
                            <div class="mt-2 grid grid-cols-1">
                                <select id="is_customer" 
                                    ...
                                    <option value="1" {{ old('is_customer', $contact->is_customer ? '1' : '0') == '1' ? 'selected' : '' }}>Yes</option>
                                    <option value="0" {{ old('is_customer', $contact->is_customer ? '1' : '0') == '0' ? 'selected' : '' }}>No</option>
                                </select>
                                <svg viewBox="0 0 16 16" fill="currentColor" class="pointer-events-none col-start-1 row-start-1 mr-2 size-5 self-center justify-self-end text-gray-500 sm:size-4 dark:text-gray-400">
                                    <path d="M4.22 6.22a.75.75 0 0 1 1.06 0L8 8.94l2.72-2.72a.75.75 0 1 1 1.06 1.06l-3.25 3.25a.75.75 0 0 1-1.06 0L4.22 7.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" fill-rule="evenodd" />
                                </svg>
                            </div>
                            @error('is_customer')<p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                        </div>

                        {{-- Is Supplier --}}
                        <div>
                            <label for="is_supplier" class="block text-sm/6 font-medium text-gray-900 dark:text-white">Supplier <span class="text-red-500">*</span></label>
                            <div class="mt-2 grid grid-cols-1">
                                <select id="is_supplier" 
                                    ...
                                    <option value="0" {{ old('is_supplier', $contact->is_supplier ? '1' : '0') == '0' ? 'selected' : '' }}>No</option>
                                    <option value="1" {{ old('is_supplier', $contact->is_supplier ? '1' : '0') == '1' ? 'selected' : '' }}>Yes</option>
                                </select>
                                <svg viewBox="0 0 16 16" fill="currentColor" class="pointer-events-none col-start-1 row-start-1 mr-2 size-5 self-center justify-self-end text-gray-500 sm:size-4 dark:text-gray-400">
                                    <path d="M4.22 6.22a.75.75 0 0 1 1.06 0L8 8.94l2.72-2.72a.75.75 0 1 1 1.06 1.06l-3.25 3.25a.75.75 0 0 1-1.06 0L4.22 7.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" fill-rule="evenodd" />
                                </svg>
                            </div>
                            @error('is_supplier')<p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div>
                                ...
                                <x-ui.toggle 
                                    name="is_active"
                                    :checked="old('is_active', $contact->is_active ? '1' : '0') == '1'"
                                />
                            </div>
                        </div>
        ...
        <div class="mt-8 flex flex-col sm:flex-row items-center justify-end gap-3 px-4 sm:px-0">
            <a href="{{ route('admin.administrator.contacts.index') }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-md bg-white dark:bg-white/10 px-4 py-2 text-sm font-semibold text-gray-900 dark:text-white shadow-xs ring-1 ring-gray-300 ring-inset dark:ring-white/10 hover:bg-gray-50 dark:hover:bg-white/20">
                Cancel
            </a>
            <button type="submit" name="action" value="save_and_stay" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-md bg-white dark:bg-white/10 px-4 py-2 text-sm font-semibold text-gray-900 dark:text-white shadow-xs ring-1 ring-gray-300 ring-inset dark:ring-white/10 hover:bg-gray-50 dark:hover:bg-white/20">
                Update & Continue Editing
            </button>
            <button type="submit" name="action" value="save" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-md bg-[var(--color-accent)] px-6 py-2 text-sm font-semibold text-white shadow-xs ring-1 ring-inset ring-[var(--color-accent)] hover:opacity-90 transition-opacity">
                Update
            </button>
        </div>
    </form>
