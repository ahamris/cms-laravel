<x-layouts.admin title="Add Translation">
<div>
        ...
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <p>Create a new key/value translation entry</p>
                </div>
            </div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    ...
                    <div class="md:col-span-3">
                        ...
                        <select class="block bg-white w-full px-3 py-2 border border-gray-200 rounded-md text-sm focus:outline-none @error('locale') border-red-300 @enderror" id="locale" name="locale" required>
                            ...
                            <option value="nl" {{ old('locale') == 'nl' ? 'selected' : '' }}>Dutch (NL)</option>
                            <option value="de" {{ old('locale') == 'de' ? 'selected' : '' }}>German (DE)</option>
                            <option value="fr" {{ old('locale') == 'fr' ? 'selected' : '' }}>French (FR)</option>
                            <option value="es" {{ old('locale') == 'es' ? 'selected' : '' }}>Spanish (ES)</option>
                            <option value="it" {{ old('locale') == 'it' ? 'selected' : '' }}>Italian (IT)</option>
                        </select>
                        @error('locale')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="mt-6">
                    ...
                    <textarea class="block bg-white w-full px-3 py-2 border border-gray-200 rounded-md text-sm focus:outline-none @error('translation_value') border-red-300 @enderror"
                              ...
                              placeholder="Enter the translated text...">{{ old('translation_value') }}</textarea>
                    ...
                    <div class="bg-gray-50 rounded-md p-4 border border-gray-200">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                            <div>
                                ...
                                <div id="preview-key" class="text-xs text-gray-500 mt-1">-</div>
                            </div>
                            <div>
                                <div class="text-sm font-medium text-gray-700">Locale:</div>
                                <div id="preview-locale" class="text-xs text-gray-500 mt-1">-</div>
                            </div>
                            <div>
                                <div class="text-sm font-medium text-gray-700">Group:</div>
                                <div id="preview-group" class="text-xs text-gray-500 mt-1">-</div>
                            </div>
                            <div>
                                <div class="text-sm font-medium text-gray-700">Value:</div>
                                <div id="preview-value" class="text-xs text-gray-500 mt-1 break-words">-</div>
                            </div>
                        </div>
                    </div>
                </div>
