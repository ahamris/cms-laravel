<x-layouts.admin title="Edit Organization">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-bold text-zinc-900 dark:text-white mb-2">Edit Organization</h1>
            <p class="text-zinc-600 dark:text-zinc-400">Update name and logo</p>
        </div>
    ...
    <form action="{{ route('admin.organization.update', $organization) }}" method="POST" enctype="multipart/form-data" id="organization-form">
        @csrf
        @method('PUT')
            ...
            <div class="rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-8 shadow-sm">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    <div class="lg:col-span-2 space-y-6">
                        <x-ui.input id="name" name="name" label="Name" :value="old('name', $organization->name)"
                    ...
                    <div class="lg:col-span-1">
                        <x-ui.image-upload
                            ...
                            label="Logo"
                            :required="false"
                            help-text="JPG, PNG, GIF, WebP or SVG. Max 20MB. Click to replace or remove."
                            :max-size="20480"
                            size="large"
                            :current-image="$organization->logo_url"
                            :current-image-alt="$organization->name . ' logo'"
                        />
                    </div>
                </div>
            </div>
