<x-layouts.admin title="Edit Solution">
    {{-- Page Header --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-bold text-zinc-900 dark:text-white mb-2">Edit Solution</h1>
            <p class="text-zinc-600 dark:text-zinc-400">Update the solution information</p>
        </div>
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="{{ route('admin.solution.show', $solution) }}" class="inline-flex items-center gap-2 rounded-md bg-white dark:bg-white/10 px-4 py-2 text-sm font-semibold text-gray-900 dark:text-white shadow-xs ring-1 ring-gray-300 ring-inset dark:ring-white/10 hover:bg-gray-50 dark:hover:bg-white/20">
    ...
    <form action="{{ route('admin.solution.update', $solution) }}" method="POST" enctype="multipart/form-data" id="solution-form">
        @csrf
        @method('PUT')
            ...
            <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <x-ui.input
                                    id="anchor"
                                    name="anchor"
                                    :value="old('anchor', $solution->anchor)"
                                    label="Anchor ID"
                            ...
                            <div>
                                <x-ui.input
                                    id="nav_title"
                                    name="nav_title"
                                    :value="old('nav_title', $solution->nav_title)"
                        ...
                        <div>
                            <x-ui.input
                                id="link_text"
                                name="link_text"
                                :value="old('link_text', $solution->link_text)"
                                label="Link Text"
                        ...
                        <div>
                            <x-ui.textarea
                                id="subtitle"
                                name="subtitle"
                                :value="old('subtitle', $solution->subtitle)"
                                label="Subtitle"
                        ...
                        <div>
                            <x-ui.textarea
                                id="short_body"
                                name="short_body"
                                :value="old('short_body', $solution->short_body)"
                                label="Short Body"
                        ...
                        <div>
                            ...
                            <div class="mt-2">
                                <x-editor 
                                    id="long_body" 
                                    name="long_body" 
                                    :value="old('long_body', $solution->long_body)"
                        ...
                        <div>
                            ...
                            <div id="list-items-container" class="space-y-2">
                                @if($solution->list_items && count($solution->list_items) > 0)
                                    @foreach($solution->list_items as $index => $item)
                                        <div class="flex gap-2">
                                            <input type="text"
                                                   name="list_items[{{ $index }}]"
                                                   value="{{ old("list_items.{$index}", $item) }}"
                        ...
                        <div>
                            ...
                            <div id="faq-container" class="space-y-4">
                                @forelse($faqItems as $idx => $faqItem)
                                <div class="faq-row rounded-lg border border-gray-200 dark:border-white/10 p-4 bg-gray-50/50 dark:bg-white/5 space-y-3">
                                    <div>
                                        <input type="text" name="faq[{{ $idx }}][question]" placeholder="Question"
                                               value="{{ old("faq.{$idx}.question", is_array($faqItem) ? ($faqItem['question'] ?? '') : '') }}"
                                        ...
                                        @error("faq.{$idx}.question")<p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                                    </div>
                                    <div>
                                        <textarea name="faq[{{ $idx }}][answer]" rows="2" placeholder="Answer"
                                                  class="block w-full rounded-md bg-white px-3 py-2 text-sm text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-[var(--color-accent)] dark:bg-white/5 dark:text-white dark:outline-white/10 dark:placeholder:text-gray-500">{{ old("faq.{$idx}.answer", is_array($faqItem) ? ($faqItem['answer'] ?? '') : '') }}</textarea>
                                        @error("faq.{$idx}.answer")<p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                                    </div>
                    ...
                    <div class="space-y-6">
                        <div>
                            <x-ui.textarea
                                ...
                                :value="old('testimonial_quote', $solution->testimonial_quote)"
                                label="Quote"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <x-ui.input
                                    ...
                                    :value="old('testimonial_author', $solution->testimonial_author)"
                                    label="Author Name"
                            ...
                            <div>
                                <x-ui.input
                                    ...
                                    :value="old('testimonial_company', $solution->testimonial_company)"
                                    label="Company"
                ...
                <div class="rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-6">
                    ...
                    <div class="space-y-6">
                        {{-- Meta Title --}}
                        @php
                            $metaKeywordsTags = array_values(array_filter(array_map('trim', explode(',', (string) old('meta_keywords', $solution->meta_keywords ?? '')))));
                        @endphp
                        ...
                        <div>
                            <x-ui.textarea
                                ...
                                :value="old('meta_description', $solution->meta_description)"
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="flex gap-4">
                                <x-ui.radio
                                    name="image_position"
                                    value="left"
                                    :checked="old('image_position', $solution->image_position) === 'left'"
                                    label="Left"
                                    color="primary"
                                />
                                <x-ui.radio
                                    name="image_position"
                                    value="right"
                                    :checked="old('image_position', $solution->image_position) === 'right'"
                                    label="Right"
                                    color="primary"
                                />
                            </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div>
                                ...
                                <x-ui.toggle 
                                    name="is_active"
                                    :checked="old('is_active', $solution->is_active)"
                                />
                            </div>
                        </div>
                    </div>
    ...
    <script>
    @php
        $existingItems = $solution->list_items ?? [];
        $listItemIndex = count($existingItems) > 0 ? count($existingItems) : 1;
        $faqItemsCount = count($faqItems);
        $faqIndexStart = $faqItemsCount > 0 ? $faqItemsCount : 1;
    @endphp
    let listItemIndex = {{ $listItemIndex }};
    let faqIndex = {{ $faqIndexStart }};
