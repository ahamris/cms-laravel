@if(in_array('view', $actions))
    <x-button 
        ...
        icon="eye" 
        title="View"
        href="{{ route('admin.content.solution.show', $item->id) }}"
    ></x-button>
@endif
...
@if(in_array('edit', $actions))
    <x-button 
        ...
        icon="edit" 
        title="Edit"
        href="{{ route('admin.content.solution.edit', $item->id) }}"
    ></x-button>
@endif
    ...
    <x-ui.modal modal-id="delete-modal-{{ $item->id }}" alpine-show="showDeleteModal{{ $item->id }}" size="sm">
        ...
        <div class="space-y-4">
            ...
            <div class="bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4 space-y-2">
                ...
                @if($item->anchor)
                <div class="text-sm">
                    <span class="text-zinc-500 dark:text-zinc-400 font-medium">Anchor:</span>
                    <span class="text-zinc-700 dark:text-zinc-300 ml-1 font-mono text-xs">#{{ $item->anchor }}</span>
                </div>
                @endif
            </div>
        </div>
