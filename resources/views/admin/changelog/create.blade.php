<x-layouts.admin title="Create Changelog Entry">
<div class="space-y-6">
    {{-- Header --}}
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col gap-1">
                <h2>Create Changelog Entry</h2>
                <p>Add a new changelog entry</p>
            </div>
        </div>
    ...
    <div class="bg-gray-50/50 rounded-md border border-gray-200">
        <form action="{{ route('admin.changelog.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select id="status" 
                                ...
                                <option value="new" {{ old('status') == 'new' ? 'selected' : '' }}>New Feature</option>
                                <option value="improved" {{ old('status') == 'improved' ? 'selected' : '' }}>Improvement</option>
                                <option value="fixed" {{ old('status') == 'fixed' ? 'selected' : '' }}>Bug Fix</option>
                                <option value="api" {{ old('status') == 'api' ? 'selected' : '' }}>API Update</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
            ...
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50/80 flex justify-between rounded-b-md">
                <a href="{{ route('admin.changelog.index') }}" class="px-5 py-2 rounded-md bg-white border border-gray-200 text-gray-700 text-sm hover:bg-gray-50 transition-colors duration-200">
                    Cancel
                </a>
