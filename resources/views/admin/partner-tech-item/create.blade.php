<x-layouts.admin title="Add Partner / Tech Stack">
    <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-lg font-semibold text-gray-900">Add record</h2>
                <p class="text-sm text-gray-500">One record per type. Add link items in the sidebar.</p>
            </div>
        </div>
    ...
    <form action="{{ route('admin.partner-tech-item.store') }}" method="POST" enctype="multipart/form-data" id="partner-tech-form">
        @csrf
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-5">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="e.g. Partners, Tech Stack" required class="w-full px-3 py-2 text-sm border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-primary @error('name') border-red-500 @enderror">
                            @error('name')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label for="type" class="block text-xs font-medium text-gray-700 mb-1">Type <span class="text-red-500">*</span></label>
                            <select id="type" name="type" required class="w-full px-3 py-2 text-sm border border-gray-200 rounded-md focus:outline-none @error('type') border-red-500 @enderror">
                                @if(!in_array(0, $existingTypes ?? []))<option value="0" {{ old('type') == '0' ? 'selected' : '' }}>Partner</option>@endif
                                @if(!in_array(1, $existingTypes ?? []))<option value="1" {{ old('type', '1') == '1' ? 'selected' : '' }}>Tech Stack</option>@endif
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" id="title" name="title" value="{{ old('title') }}" placeholder="Section title" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-md focus:outline-none">
                            @error('title')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label for="description" class="block text-xs font-medium text-gray-700 mb-1">Description</label>
                            <textarea id="description" name="description" rows="2" placeholder="Optional" class="w-full px-3 py-2 text-sm border border-gray-200 rounded-md focus:outline-none resize-none">{{ old('description') }}</textarea>
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-lg border border-gray-200 p-4 sticky top-4">
                    ...
                    <div id="data-items-container" class="space-y-3 max-h-[calc(100vh-12rem)] overflow-y-auto pr-1">
                        @php
                            $oldData = old('data', []);
                            $staticPagesCreate = $staticPages ?? collect();
                        @endphp
                        @if(count($oldData))
                            @foreach($oldData as $i => $row)
                                @php $lt = $row['link_type'] ?? 'external'; $linkVal = $row['link'] ?? ''; $rowImage = $row['image'] ?? null; @endphp
                                    ...
                                    <div class="flex items-center justify-between gap-1">
                                        ...
                                        <div class="data-link-static flex-1 min-w-0" style="{{ $lt == 'external' ? 'display:none' : '' }}">
                                            <select class="data-link-select w-full px-2 py-1 text-xs border border-gray-200 rounded">
                                                ...
                                                @foreach($staticPagesCreate as $sp)
                                                    <option value="{{ $sp->slug }}" {{ $linkVal === $sp->slug ? 'selected' : '' }}>{{ Str::limit($sp->title, 20) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="flex items-center gap-1 shrink-0">
                                            <input type="number" name="data[{{ $i }}][sort_order]" value="{{ $row['sort_order'] ?? $i }}" min="0" class="w-10 px-1 py-1 text-xs border border-gray-200 rounded">
        ...
        <div class="mt-6 flex justify-end gap-3">
            ...
            <button type="submit" class="px-4 py-2 text-sm text-white bg-primary rounded-md hover:bg-primary/80">Create</button>
        </div>
    </form>
