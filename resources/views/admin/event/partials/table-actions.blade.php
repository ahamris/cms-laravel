@if(in_array('view', $actions))
    <a href="{{ route('admin.content.event.show', $item) }}" title="View">
        <x-button variant="sky" size="sm" icon="eye" title="View"></x-button>
    </a>
@endif

@if(in_array('edit', $actions))
    <a href="{{ route('admin.content.event.edit', $item) }}" title="Edit">
...
@if(in_array('delete', $actions))
    @php
        $modelName = class_basename($this->modelClass ?? \App\Models\Event::class);
    @endphp
    <x-button
        variant="error"
        size="sm"
    ...
    <x-ui.modal modal-id="delete-modal-{{ $item->id }}" alpine-show="showDeleteModal{{ $item->id }}" size="sm">
        ...
        <div class="space-y-4">
            ...
            <div class="bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4 space-y-2">
                <div class="text-sm">
                    <span class="text-zinc-500 dark:text-zinc-400 font-medium">Event:</span>
                    <span class="text-zinc-700 dark:text-zinc-300 ml-1">{{ $item->title }}</span>
                </div>
                @if($item->start_date)
                    <div class="text-sm">
                        <span class="text-zinc-500 dark:text-zinc-400 font-medium">Date:</span>
                        <span class="text-zinc-700 dark:text-zinc-300 ml-1">{{ $item->start_date->format('M d, Y') }}</span>
                    </div>
                @endif
            </div>
        </div>
