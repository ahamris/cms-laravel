<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">Edit {{ $heading }}</h1>
            <p class="mt-1 text-zinc-600 dark:text-zinc-400">Type <code class="rounded bg-zinc-100 px-1.5 py-0.5 text-xs dark:bg-zinc-700">{{ is_object($type) ? $type->value : $type }}</code> · ID {{ $element->id }}</p>
        </div>
