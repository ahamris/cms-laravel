...
@elseif(isset($header) || isset($body) || isset($footer) || ($slot->isNotEmpty() && empty($title) && empty($value)))
    ...
    @else
        ...
        @endif
            ...
            @if(isset($header))
                <div @class([
                    'mb-2 pb-2' => !$showDividers,
                    'mb-4 pb-4 border-b border-zinc-200 dark:border-zinc-700' => $showDividers
                ])>
                    {{ $header }}
                </div>
            @endif
            ...
            <div class="card-body">
                @if(isset($body))
                    <div class="text-base leading-6 tracking-wide text-zinc-700 dark:text-zinc-300">
                        {{ $body }}
                    </div>
            ...
            @if(isset($footer))
                <div @class([
                    'mt-2 pt-2' => !$showDividers,
                    'mt-4 pt-4 border-t border-zinc-200 dark:border-zinc-700' => $showDividers
                ])>
                    {{ $footer }}
                </div>
            @endif
        ...
        @endif
            ...
            <div class="flex items-start gap-4 mb-4">
                @if($icon)
                    ...
                    @endphp
                    <div
                        class="{{ $iconSize }} rounded-lg {{ $colorClasses['bg'] }} {{ $colorClasses['bgDark'] }} flex items-center justify-center flex-shrink-0 shadow-sm">
                        <i
                            class="fas fa-{{ $icon }} {{ $colorClasses['text'] }} {{ $colorClasses['textDark'] }} {{ $iconTextSize }}"></i>
                    </div>
                @endif
                <div class="flex-1">
                    @if($title)
            ...
            @if($actionText && $actionUrl && $actionUrl !== 'javascript:void(0)')
                ...
                @endphp
                <a href="{{ $actionUrl }}"
                    class="flex items-center justify-between {{ $actionTextSize }} font-medium text-zinc-600 dark:text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400 group transition-colors duration-200"
