...
<header class="bg-indigo-600 dark:bg-indigo-900 {{ $isSticky ? 'sticky top-0 z-50' : '' }}">
  <nav aria-label="Global" class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8">
        ...
        @foreach($megaMenuData as $index => $menuItem)
          ...
          @if(!empty($menuItem['children']))
            <x-dynamic-component :component="'front.flyout-menus.' . $flyoutName" :item="$menuItem" :index="$index"
              triggerClass="text-sm/6 font-semibold text-white" />
          @else
            <a href="{{ $menuItem['url'] ?? '#' }}" class="text-sm/6 font-semibold text-white">
              {{ $menuItem['title'] }}
            </a>
          @endif
        @endforeach
    ...
    <div class="hidden lg:flex lg:flex-1 lg:justify-end">
      <x-front.header-actions :light="true" />
    </div>
  </nav>
  <el-dialog>
