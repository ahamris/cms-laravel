                ...
                <div class="flex items-center justify-between">
                    @if(!empty($item['footer_action_1_text']))
                        <a href="{{ $item['footer_action_1_url'] ?? '#' }}"
                            ...
                            {{ $item['footer_action_1_text'] }} <span aria-hidden="true">&rarr;</span>
                        </a>
                    @endif
                    @if(!empty($item['footer_action_2_text']))
                        <a href="{{ $item['footer_action_2_url'] ?? '#' }}"
                            class="text-sm/6 font-semibold text-gray-900 dark:text-white hover:text-gray-700">
                            {{ $item['footer_action_2_text'] }} <span aria-hidden="true">&rarr;</span>
                        </a>
                    @endif
                </div>
            </div>
