...
- **Content (public, no auth)** — All content endpoints are public. CORS is handled by Laravel’s built-in CORS middleware using `config/cors.php` (env `CORS_ALLOWED_ORIGINS` or `FRONTEND_ALLOWED_ORIGINS`). Empty list = all origins allowed.
