---
title: OPCM Admin UI Design System
version: "1.0"
date: 2026-03-26
stack:
  - Tailwind CSS v3
  - Livewire 3
  - Alpine.js v3
purpose: Standard UI system for Cursor AI IDE and all frontend contributors
scope: Admin interface only
status: Canonical source of truth
---

# OPCM Admin UI Design System for Cursor

This file is the canonical Markdown design system for the OPCM admin interface. It is intended for direct use inside Cursor AI IDE so UI changes can be generated consistently across the codebase.

**Operational checklist (PRs):** [admin-ds-compliance-checklist.md](./admin-ds-compliance-checklist.md)

## Cursor system instruction

Apply this document as a hard UI standard when generating or editing:
- Blade templates
- Livewire views
- Alpine.js interactions
- reusable view components
- admin list, detail, create, edit, dashboard, drawer, modal, toast, table, and form screens

### Non-negotiable rules for Cursor
- Reuse existing components first before creating new ones.
- Never use arbitrary Tailwind color classes such as `bg-[#...]` or `text-[#...]`.
- Never introduce a new UI library without architecture review.
- Use semantic tokens from `tailwind.config.js` only.
- Follow the page patterns, button hierarchy, spacing scale, and overlay rules exactly.
- Use modals only for destructive confirmation.
- Use drawers for edit, preview, create, settings, and import flows.
- Use toasts for transient feedback only.
- Ensure all output works across desktop-first admin layouts and the defined mobile PWA fallback.
