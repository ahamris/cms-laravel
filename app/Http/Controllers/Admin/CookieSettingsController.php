...
class CookieSettingsController extends AdminBaseController
{
    ...
    {
        // Get available legal pages for dropdown
        $legalPages = \App\Models\Legal::where('is_active', true)
            ->select('id', 'title', 'slug')
            ->orderBy('title')
            ->get();

        // Get available static pages for dropdown
        $staticPages = \App\Models\StaticPage::where('is_active', true)
            ->select('id', 'title', 'slug')
            ->orderBy('title')
            ->get();

        return view('admin.settings.cookie.index', compact('legalPages', 'staticPages'));
    }
    ...
    public function update(Request $request)
    {
        $request->validate([
            ...
            'cookie_settings_page_type' => 'nullable|string|in:legal,static,custom',
            'cookie_settings_page_id' => 'nullable|integer',
            
            'cookie_policy_url' => 'nullable|string|max:500',
            'cookie_policy_page_type' => 'nullable|string|in:legal,static,custom',
        ...
        try {
            ...
            // Handle cookie settings URL based on page type
            if ($request->cookie_settings_page_type === 'legal' && $request->cookie_settings_page_id) {
                $legal = \App\Models\Legal::find($request->cookie_settings_page_id);
                $this->updateSetting('cookie_settings_url', $legal ? route('legal.show', $legal->slug) : $request->cookie_settings_url);
            } elseif ($request->cookie_settings_page_type === 'static' && $request->cookie_settings_page_id) {
                $static = \App\Models\StaticPage::find($request->cookie_settings_page_id);
                $this->updateSetting('cookie_settings_url', $static ? route('static.show', $static->slug) : $request->cookie_settings_url);
            } else {
            ...
            $this->updateSetting('cookie_settings_page_id', $request->cookie_settings_page_id);
            
            // Handle cookie policy URL based on page type
            if ($request->cookie_policy_page_type === 'legal' && $request->cookie_policy_page_id) {
                $legal = \App\Models\Legal::find($request->cookie_policy_page_id);
                $this->updateSetting('cookie_policy_url', $legal ? route('legal.show', $legal->slug) : $request->cookie_policy_url);
            } elseif ($request->cookie_policy_page_type === 'static' && $request->cookie_policy_page_id) {
                $static = \App\Models\StaticPage::find($request->cookie_policy_page_id);
                $this->updateSetting('cookie_policy_url', $static ? route('static.show', $static->slug) : $request->cookie_policy_url);
            } else {
            ...
            $this->updateSetting('cookie_policy_page_id', $request->cookie_policy_page_id);
